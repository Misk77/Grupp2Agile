...
// make an instance of this there you wanna play then call the playbackground with a string path to the file you wanna play
		...
		while (running) {
			...
			if (inputStream == null && playSong) {
				...
				try {
					...
					clip.loop(Clip.LOOP_CONTINUOUSLY); // change from clip.loop(10); to LOOP_CONTINUOUSLY
				} catch (Exception e) {
	...
	public void playBackGround(String string) // call to play .wav file String is the path to the wav file and then add
												// .wav from url variable
	{
		if (clip != null) {
			this.clip.stop();
			this.clip.close();
